...
from models.hcmut_entities import hcmut_entities
my_entities = hcmut_entities
...
class EntityTupleSearcher:
    ...
    def dfs_ent(self,
        ...
        for pred_ent in my_entities:
            ...
            # Giả sử logprob mặc định là 0 (hoặc bạn có thể tuỳ chỉnh)
            logprob = 0
            heapq.heappush(collected_ent_heap, [min(cur_logprobs + [logprob]), pred_ent])
            while len(collected_ent_heap) > n:
                heapq.heappop(collected_ent_heap)
